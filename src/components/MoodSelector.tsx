                    ...
                    {MOODS.map((m) => {
                        ...
                        return (
                            <button
                                ...
                                className={cn(
                                    "relative flex flex-col items-center justify-center p-3 rounded-xl transition-all duration-300 isolate group",
                                    isSelected ? "z-10" : "z-0"
                                )}
                            >
                                {isSelected && (
                                    <div
                                        className={cn(
                                            "absolute inset-0 rounded-xl z-[-1] animate-in fade-in zoom-in-95 duration-300", 
                                            m.color.split(' ')[0], 
                                ...
                                <span
                                    className={cn(
                                        "text-2xl mb-1 filter drop-shadow-sm select-none transition-transform duration-300",
                                        isSelected ? "scale-125 -translate-y-0.5" : "group-hover:scale-110"
                                    )}
                                >
                                    {m.emoji}
                                </span>
            ...
            <div>
                <div className="flex items-center justify-between mb-3">
                    ...
                    {energy && (
                        <span
                            className={cn(
                                "text-xs font-bold px-2 py-0.5 rounded-full flex items-center gap-1 animate-in fade-in slide-in-from-top-1 duration-300",
                ...
                <div className="relative h-14 bg-surface/30 rounded-xl border border-surfaceHighlight flex items-center px-4 group overflow-hidden">
                    ...
                    <div className="absolute left-4 right-4 top-1/2 -translate-y-1/2 h-2 rounded-full pointer-events-none">
                        <div
                            className={cn(
                                "h-full rounded-full transition-all duration-300 ease-out shadow-[0_0_15px_rgba(0,0,0,0.3)]",
